...
Process {
    ...
    Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS_Drivers.ps1.tag" -Value "Installed"

    # Start logging
    Start-Transcript "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS_Drivers.log"
    ...
    $queries | ForEach-Object {
        ...
        ((New-Object -ComObject Microsoft.Update.Session).CreateupdateSearcher().Search($_)).Updates | ForEach-Object {
            ...
            if ($_.Title -notmatch "Preview" -and $_.Categories[0].Name -notmatch "Upgrades" -and $_.Title -notmatch "Firmware - 1.61.0.0") { [void]$WUUpdates.Add($_) }
        }
